...
CMessageDlg::CMessageDlg(CWnd* pParent, CString message, UINT type, int timeout_ms)
	: CSCThemeDlg(IDD_MESSAGE, pParent)
{
	set(message, type, timeout_ms);
}

void CMessageDlg::set(CString message, UINT type, int timeout_ms)
{
	m_message = message;
	m_type = type;
...
BOOL CMessageDlg::OnInitDialog()
{
	...
	m_titlebar_height = 28;
	set_titlebar_height(m_titlebar_height);
	m_sys_buttons.set_button_width(32);
...
BOOL CMessageDlg::PreTranslateMessage(MSG* pMsg)
{
	...
	return CSCThemeDlg::PreTranslateMessage(pMsg);
}
