...
[nFTDClient.exe가 이전 버전일 경우 에러 발생 기능\
- remote에서 drag&drop 할 경우


[수정할 내용]
- 전송결과를 콤마로 구분하지 말고 공백으로 구분
...
- 앱 시작 시 경로가 잘못된 즐겨찾기라도 이 앱 또는 탐색기 등에서 해당 폴더를 실제 존재하도록 수정한 후
  즐겨찾기 폴더를 더블클릭하면 해당 폴더로 이동되지만 색상은 여전히 정상으로 돌아오지 않고 빨간색으로 표시된다.
